    ...
    if data.split("-", 1)[0] == "BATCHS":
        ...
        async for msg in client.iter_messages(int(f_chat_id), int(l_msg_id), int(f_msg_id)):
            if msg.media:
                ...
                try:
                    await msg.copy(message.chat.id, caption=f_caption, protect_content=True if protect == "/pbatchs" else False)
                except FloodWait as e:
                    await asyncio.sleep(e.x)
                    await msg.copy(message.chat.id, caption=f_caption, protect_content=True if protect == "/pbatchs" else False)
                except Exception as e:
                ...
                try:
                    await msg.copy(message.chat.id, protect_content=True if protect == "/pbatchs" else False)
                except FloodWait as e:
                    await asyncio.sleep(e.x)
                    await msg.copy(message.chat.id, protect_content=True if protect == "/pbatchs" else False)
                except Exception as e:
